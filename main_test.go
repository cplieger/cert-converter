...
// ACTS on process.OpenMounts' verdict: run() must return 1 before it builds a scanner
...
func TestRun_refuses_to_start_when_a_required_volume_is_missing(t *testing.T) {
	...
	os.Args = []string{"cert-watcher"}
	requiredVolumes = []process.Mount{
		{Role: process.RoleInput, Path: t.TempDir()},
		{Role: process.RoleOutput, Path: absentOutput},
	}
	t.Cleanup(func() {
		os.Args = prevArgs
		slog.SetDefault(prevLogger)
