...
| `INPUT_EXCLUDE_PATHS` | Comma-separated paths under `/input`, relative to the mount, that this app must not convert. A path naming a directory covers everything beneath it; a path naming a file covers that file. Use it when the mount also holds certificates that are not yours to convert, so no PFX bundle or PEM copy of their private keys is produced. An excluded path is treated as **out of scope, not absent**: it is still enumerated, and any artifacts it previously produced stay in `/output` untouched even under `OUTPUT_LIFECYCLE=sync`, so a mistyped entry cannot delete anything. Orphan cleanup keeps working normally for every other source, which is the difference from restricting access by permissions. Excluding one spelling of a name never strands another, so excluding `site.crt` lets a `site.pfx` beside it convert. An entry that is absolute, traverses upward, or names the mount itself is reported and ignored, because it could never match anything and would silently exclude nothing. When every source found is excluded, startup-time conversion produces nothing and a WARN says so. | _(unset)_ | No |
