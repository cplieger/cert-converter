...
| `INPUT_EXCLUDE_PATHS` | Comma-separated paths under `/input`, relative to the mount, that this app must not convert. A path naming a directory covers everything beneath it; a path naming a file covers that file. Use it when the mount also holds certificates that are not yours to convert, so no PFX bundle or PEM copy of their private keys is produced. Excluding a path narrows the scope this app manages, so artifacts it produced earlier become ordinary orphans and `OUTPUT_LIFECYCLE` decides them, exactly as it does when you switch a format off: `keep` is silent, `warn` reports them, and `sync` removes them. Only `sync` ever deletes. Orphan cleanup keeps working normally for every other source, which is the difference from restricting access by permissions. Excluding one spelling of a name never strands another, so excluding `site.crt` lets a `site.pfx` beside it convert. An entry that is absolute, traverses upward, or names the mount itself is reported and ignored, and an unusable entry also forces `OUTPUT_LIFECYCLE=warn`, so a value this app could only partly parse never authorizes a deletion. When every source found is excluded, the scan produces nothing and a WARN says so. | _(unset)_ | No |
...
| Mount | Description |
| --- | --- |
| `/input` | Certificate source directory (read-only). A source is a PEM pair, `<name>.crt` with its private key as the sibling `<name>.key` in the same directory, or, once `INPUT_PFX_PASSWORD` is configured, a PKCS#12 bundle named `<name>.pfx` or `<name>.p12`. When one name has both, the PEM pair wins and the bundle is skipped. Files with any other extension are ignored, so a certbot-style directory of `fullchain.pem`/`privkey.pem` produces no output and logs `no certificate sources found under the input root`. **The whole tree is scanned recursively, to any depth**, so every source anywhere under the mount is converted. To scope that, mount the specific directory you want converted, or list the paths to skip in `INPUT_EXCLUDE_PATHS`. Do not use permissions to exclude a sub-path: an `/input` path this app cannot read is counted as unreadable, which is health-neutral but disables orphan cleanup for every scan and reports a standing warning, whereas an excluded path leaves cleanup working. Must be readable by the UID in `user:`; an ACME client's certificate directory is often root-owned and mode `0700`, so `chgrp`/`chmod` it for that UID or run the container as a UID that can read it. |
